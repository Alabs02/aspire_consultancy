...
import { withAdmin } from '../../Store/admin';
...
import './AdminMiniHeader.css';

const MiniHeader = () => {

  const history = useHistory();
  const userProfile = useRecoilValue(withAdmin);
  const [isVisible, setIsVisible] = useState(false); 
  const [isOpen, setIsOpen] = useState(false);

  return (
    <Fragment>
      <div className="flex justify-between w-full items-center bg-white-pure border-0 pr-4 pl-4 md:pl-0 md:pr-10 border-blue-500">
        <div className="search relative w-2/5 hidden md:block">
          <BiSearch className="absolute search__icon left-5 text-secondary-light" size={18} />
